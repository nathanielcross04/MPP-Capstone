...
anim_obj <- animate(
  ...
  width    = 700,
  height   = 700,
  renderer = gifski_renderer(file.path(OUTPUT_DIR, "radar_plot_2CS_2000_both.gif")),
  bg       = "white"
)
...
anim_obj <- animate(
  ...
  width    = 700,
  height   = 700,
  renderer = gifski_renderer(file.path(OUTPUT_DIR, "radar_plot_2CS_blank.gif")),
  bg       = "white"
)

message("GIF saved to: ", file.path(OUTPUT_DIR, "radar_plot_2CS_blank.gif"))
...
interp_poly_c1 <- interp_poly %>% filter(cluster == "1")
interp_pts_c1  <- interp_pts  %>% filter(cluster == "1")
...
p_radar_red <- ggplot() +
  ...
  geom_segment(data = spoke_axes,
               ...
               linewidth = 1.2, show.legend = FALSE) +
  scale_color_identity() +
  geom_polygon(data = interp_poly_c1,
               aes(x = x, y = y, group = interaction(frame_id, cluster),
                   fill = cluster_color),
               alpha = 0.15, color = NA, show.legend = FALSE) +
  scale_fill_identity() +
  geom_path(data = interp_poly_c1,
            aes(x = x, y = y, group = interaction(frame_id, cluster),
                color = cluster_color),
            linewidth = 0.8, show.legend = FALSE) +
  geom_point(data = interp_pts_c1,
             aes(x = x, y = y, color = CLUSTER_COLORS[cluster]),
             size = 5, show.legend = FALSE) +
  geom_text(data = labels_df,
            aes(x = x, y = y, label = label, color = color),
...
anim_obj <- animate(
  ...
  width    = 700,
  height   = 700,
  renderer = gifski_renderer(file.path(OUTPUT_DIR, "radar_plot_2CS_red.gif")),
  bg       = "white"
)

message("GIF saved to: ", file.path(OUTPUT_DIR, "radar_plot_2CS_red.gif"))