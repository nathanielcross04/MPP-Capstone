...
build_widget <- function(year, raw) {
  ...
  # ── Layout: prioritize state-to-centroid distances ───────────
  
  # Step 1: place centroids via MDS on just the 2-centroid distance matrix
  centroid_policy <- as.matrix(centroid_nodes[, POLICY_COLS])
  centroid_dist   <- as.numeric(dist(centroid_policy, method = "euclidean"))
  
  # Place centroids on opposite ends of the x-axis, scaled by their distance
  # All other axes are zero for centroids — states spread around them in y/z
  c1_pos <- c(-0.5, 0, 0)
  c2_pos <- c( 0.5, 0, 0)
  
  # Step 2: for each state, compute its position as:
  #   - x: lerp between its centroid's x and the other centroid's x,
  #         weighted by distance to own centroid (closer = nearer centroid x)
  #   - y/z: spread within-cluster peers using 2D MDS on within-cluster
  #           state-to-state distances, scaled to not overwhelm x placement
  
  cluster_ids <- as.character(nodes$state_cluster_id[state_idx])
  unique_clusts <- unique(cluster_ids)
  
  # Compute within-cluster 2D MDS for y/z spread per cluster
  yz_coords <- matrix(0, nrow = length(state_idx), ncol = 2)
  
  for (cl in unique_clusts) {
    cl_mask  <- which(cluster_ids == cl)
    cl_state_idx <- state_idx[cl_mask]
    ...
    cl_policy <- policy_matrix[cl_state_idx, ]
    cl_dist   <- dist(cl_policy, method = "euclidean")
    
    set.seed(42)
    cl_mds <- tryCatch(
      cmdscale(cl_dist, k = 2),
      error = function(e) matrix(0, nrow = length(cl_mask), ncol = 2)
    )
    
    # Rescale y/z within cluster to [-0.4, 0.4] range
    rescale_yz <- function(x) {
      rng <- range(x)
      if (diff(rng) == 0) return(x * 0)
      0.8 * (x - rng[1]) / diff(rng) - 0.4
    }
    cl_mds_scaled <- apply(cl_mds, 2, rescale_yz)
    yz_coords[cl_mask, ] <- cl_mds_scaled
  }
  
  # Step 3: compute x position from distance to own centroid
  # Normalize distances within each cluster so the furthest state
  # maps to LAYOUT_SPREAD and closest maps to 0 displacement from centroid
  x_coords <- numeric(length(state_idx))
  
  # Get per-cluster max distance for normalization
  clust_max_dist <- tapply(state_to_centroid_dists, cluster_ids, max)
  
  for (i in seq_along(state_idx)) {
    cl       <- cluster_ids[i]
    d        <- state_to_centroid_dists[i]
    max_d    <- clust_max_dist[cl]
    norm_d   <- if (max_d > 0) d / max_d else 0   # 0 = at centroid, 1 = furthest
    
    if (cl == "1") {
      # Cluster 1 centroid is on left; states spread further left as distance grows
      x_coords[i] <- c1_pos[1] - norm_d * LAYOUT_SPREAD * 0.5
    } else {
      # Cluster 2 centroid is on right; states spread further right
      x_coords[i] <- c2_pos[1] + norm_d * LAYOUT_SPREAD * 0.5
    }
  }
  
  # Assemble final coordinates
  # State rows first (1:51), centroid rows last (52:53)
  all_x <- c(x_coords, c1_pos[1], c2_pos[1])
  all_y <- c(yz_coords[, 1], 0, 0)
  all_z <- c(yz_coords[, 2], 0, 0)
  
  nodes$x <- all_x
  nodes$y <- all_y
  nodes$z <- all_z
  
  # Small jitter to separate any overlapping states
  set.seed(42)
  ...
  # ── Edges: state → cluster centroid, per-cluster median threshold ──
  ...
  for (i in seq_along(state_idx)) {
    ...
    clust <- as.character(nodes$state_cluster_id[idx_i])
    idx_c <- centroid_cluster_map[clust]
    if (state_to_centroid_dists[i] <= cluster_medians[clust]) {
...
slider_js <- '
  ...
  function showYear(yr) {
    ...
    // Lazy-load: set src only on first visit
    if (!loaded[yrStr]) {
...
# Build iframe tags — only 2000 gets src; rest use data-src for lazy loading
...
# Run this in R after saving the HTML file
