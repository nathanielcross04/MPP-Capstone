...
p_radar <- ggplot() +
  ...
  lapply(gridlines, function(gl) {
    geom_path(data = gl, aes(x = x, y = y),
              color = "#CCCCCC", linewidth = 0.5 * SCALE, linetype = "dashed",
              inherit.aes = FALSE)
  }) +
  
  geom_segment(data = spoke_axes,
               aes(x = 0, y = 0, xend = x_end, yend = y_end, color = color),
               linewidth = 1.2 * SCALE, show.legend = FALSE) +
  ...
  theme(
    ...
    plot.title  = element_text(size = 25 * SCALE,
                               ...
                               lineheight = 1.2,
                               hjust = 0,
                               margin = margin(t = 50 * SCALE, b = 15 * SCALE)),
    plot.background = element_rect(fill = "white", color = NA),
    plot.margin     = margin(t = 20 * SCALE, r = 20 * SCALE,
                             b = -30 * SCALE, l = 20 * SCALE)
  ) +
...
animate(
  ...
  renderer = av_renderer(
    file.path(OUTPUT_DIR, "radar_plot_1CS.mp4"),
    vfilter = "scale=trunc(iw/2)*2:trunc(ih/2)*2"
  ),
  bg = "white"
)

message("MP4 saved to: ", file.path(OUTPUT_DIR, "radar_plot_1CS.mp4"))
...
p_radar <- ggplot() +
  ...
  geom_path(data = interp_poly,
            aes(x = x, y = y, group = interaction(frame_id, cluster),
                color = cluster_color),
            linewidth = 0.8 * SCALE, show.legend = FALSE) +
...
animate(
  ...
  height   = OUTPUT_HEIGHT,
  renderer = av_renderer(
    file.path(OUTPUT_DIR, "radar_plot_2CS.mp4"),
    vfilter = "scale=trunc(iw/2)*2:trunc(ih/2)*2"
  ),
  bg = "white"
)

message("MP4 saved to: ", file.path(OUTPUT_DIR, "radar_plot_2CS.mp4"))